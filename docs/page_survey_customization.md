...
| 후속 수집 정책 | 행사별로 `행사 전체 만족도만`, `상담 전문가별 만족도만`, `둘 다`, `수집 안 함`을 선택할 수 있도록 확장한다 |
...
### 범위 밖 (이후)
- [x] 슬라이스 D — EXPERT 대상 설문(전문가 포탈 노출, **Phase 6 합류**)
- [x] 후속 — 행사별 만족도 수집 정책 설정(`행사 전체`, `전문가별`, `둘 다`, `수집 안 함`) (8-D `0047` events.satisfaction_policy)
- [x] 후속 — 상담 전문가별 만족도 응답 모델 및 스타트업 응답 UX (8-G `0048` — survey_scope=EXPERT, 슬롯당 1회, `ExpertSatisfactionPanel`)
- [x] 후속 — 관리자 `행사 만족도`/`전문가 만족도` 결과 화면 분리 (8-G — `ExpertSurveyReportPanel` 전문가별 집계)
- [ ] 백로그 — 익명 설문 / 리포트 PDF 출력
