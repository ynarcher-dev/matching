...
- **§2 노쇼 대체 매칭, §3 증빙사진 셀 통합: 미구현(후속 과제).** 아래 결정 메모 참고.
...
- **§3**: 셀이 이미 좁으므로(150px) 셀 내부 인라인 업로드 대신 셀의 `📷` 버튼 → 컴팩트 팝오버(`CompanyPhotoUploadPanel` 핵심 기능)로 구현하고, 상단 툴바에 "사진 미등록 셀만 보기" 토글을 둔다. 권한은 §4 권고대로 관리/스태프 우선.
